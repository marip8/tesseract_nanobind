"""
SSL shapes viewer for WebXR VR headset support.

SSL may be necessary for some systems that use WebXR. By default WebXR is not
allowed to run on insecure origins. The user will need to accept the security
warning for a self signed certificate.

Also allow all incoming addresses to connect to the server by binding to 0.0.0.0
instead of localhost. Firewalls may also need to be configured to allow incoming
connections.

Point your VR headset to the https address of the computer running the server.
For example https://192.168.1.10:8000 if the server is running on port 8000 on
the computer with IP address 192.168.1.10.
