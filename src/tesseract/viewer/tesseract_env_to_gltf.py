...
def tesseract_env_to_gltf_dict_and_buf(t_env, origin_offset=[0, 0, 0], name=None):
    ...
    for link_name in link_names:
        link_map[link_name] = t_env.getLink(link_name)
...
def _append_link_recursive(
    gltf_dict, gltf_buf_io, link_map, joint_map, link_name, shapes_mesh_inds=None
):
    ...
    for visual in link.visual:
        visual_i += 1
        _, visual_ind = _append_link_visual(
    ...
    for j in child_joints:
        ...
        parentjoint_node, parentjoint_ind = _append_node(
            gltf_dict, _tf_joint, "jointparent_" + j.getName()
        )
        joint_extras = {
            "tesseract_joint": {
                "axis": list(j.axis.flatten()),
                "type": j.type.value,
                "name": j.getName(),
            }
        }
        joint_node, joint_id = _append_node(
            gltf_dict, name="joint_" + j.getName(), extras=joint_extras
        )
        parentjoint_node["children"] = [joint_id]
        _, j_link_ind = _append_link_recursive(
            gltf_dict,
            gltf_buf_io,
            link_map,
            joint_map,
...
def _append_accessor(gltf_dict, gltf_buf_io, dat):
    ...
    _, bufview_ind = _append_dict_list(
        gltf_dict,
        "bufferViews",
        {"buffer": 0, "byteLength": byteLength, "byteOffset": byteOffset},
    )

    gltf_buf_io.write(dat)

    accessor, accessor_ind = _append_dict_list(
        gltf_dict,
        "accessors",
        {
...
def _append_bufview(gltf_dict, gltf_buf_io, dat):
    ...
    bufview_dict, bufview_ind = _append_dict_list(
        gltf_dict,
        "bufferViews",
        {"buffer": 0, "byteLength": byteLength, "byteOffset": byteOffset},
    )
    ...
    for i in range(len(joint_names)):
        ...
        if joint_type in (1, 2):
            ...
            animation["samplers"].append(
                {"input": t_ind, "output": s_ind, "interpolation": "LINEAR"}
            )

            animation["channels"].append(
                {
                    "sampler": i,
                    "target": {"node": joint_inds[joint_name], "path": "rotation"},
                }
            )
        elif joint_type == 3:
            ...
            animation["channels"].append(
                {
                    "sampler": i,
                    "target": {"node": joint_inds[joint_name], "path": "translation"},
                }
            )
...
def _load_shapes():
    geometries = json.loads(
        pkgutil.get_data("tesseract.viewer.resources", "geometries.json")
    )["geometries"]
